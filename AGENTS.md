...
- Every OpenAI call goes through `access.openai_response` (embeddings: `access.openai_embedding`):
  it refuses at an access pass's cap, charges the pass from reported usage, and fails closed on the
  public demo when no pass is set. A new model call site must use it.
  Passes, admin and hosting: README "Deploy to Render".
