...
export interface RunLabel { short: string }

/**
 * Hex run ids ("d428e213e6") name nothing. Number runs in the order they were made — oldest is
 * Run 1, because every bundled run shares one company name and History has to tell them apart. Callers keep the raw id as a tooltip so a run is still findable on disk.
 */
