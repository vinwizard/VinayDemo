...
- **Blind probes** never name the brand → **visibility**. Do you show up at all? They are asked on
  ...
  different answer on every run; the gap between fronts says whether it is real), and each
...
Tests: `conda activate visexp && python -m pytest -q` (offline; 365 passing,
